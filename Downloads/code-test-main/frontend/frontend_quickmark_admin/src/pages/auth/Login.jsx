...
const Login = ({ onLoginSuccess }) => { // Prop changed to onLoginSuccess
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:3700/api/admin/auth/login', {
                email,
                password,
            });
            ...
            if (onLoginSuccess) { // Call the success handler passed from App.jsx
                onLoginSuccess(token);
            } else {
                // Fallback direct navigation if onLoginSuccess is not provided (e.g., for testing directly)
