...
import { Filter, PlusCircle, Edit, Trash2 } from 'lucide-react'; // Import icons

const StudentsList = () => { // Renamed from Students to StudentsList for clarity based on App.jsx
    ...
    const filteredStudents = useMemo(() => {
        return students.filter(student => {
            const searchMatch =
                ...
                student.roll_number.toLowerCase().includes(searchTerm.toLowerCase()); // Use roll_number
                        ...
                        <tbody>
                            {students.map((student) => (
                                <tr key={student.student_id} className="border-b last:border-b-0 hover:bg-gray-50">
                                    ...
                                    {/* Display Department Name directly from fetched data */}
                                    <td className="py-2 px-4">{departments.find(d => d.department_id === student.department_id)?.name || 'N/A'}</td>
