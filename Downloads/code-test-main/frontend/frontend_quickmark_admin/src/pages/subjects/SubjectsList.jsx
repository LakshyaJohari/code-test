...
const SubjectsList = () => {
    // Departments state (still needed for the dropdowns in create/edit subject forms)
    const [departments, setDepartments] = useState([]);
    const [loadingDepartments, setLoadingDepartments] = useState(true); // Keep loading state
    const [departmentError, setDepartmentError] = useState(''); // Keep error state
    ...
    // Fetch Departments (for dropdowns in subject forms)
    const fetchDepartments = async () => {
        ...
        try {
            ...
            console.log('Frontend: Fetched Departments Data (for dropdowns):', response.data); // DEBUG LOG
            setDepartments(response.data);
        } catch (err) {
            console.error('Frontend: Error fetching departments:', err.response?.data?.message || err.message);
    ...
    const fetchSubjects = async () => {
        ...
        try {
            ...
            console.log('Frontend: Fetched Subjects Data:', response.data); // DEBUG LOG
            setSubjects(response.data);
        } catch (err) {
            console.error('Frontend: Error fetching subjects:', err.response?.data?.message || err.message);
    ...
    useEffect(() => {
        console.log('Frontend: SubjectsList component mounting/updating'); // DEBUG LOG
        fetchDepartments(); // Still need departments for subject forms
        fetchSubjects();
    }, []);

    // Debug logs for state changes (can be removed later)
    useEffect(() => {
        console.log('Frontend: Current Subjects State:', subjects);
    }, [subjects]);
    useEffect(() => {
        console.log('Frontend: Current Departments State:', departments);
    }, [departments]);
    ...
    const handleCreateSubject = async (e) => {
        ...
        try {
            ...
            setNewSubject({ subject_name: '', department_id: '', year: '', section: '', batch_name: '' }); // Clear form
    ...
    const handleUpdateSubject = async (e) => {
        ...
        try {
            ...
            setEditingSubject(null); // Close edit modal
    ...
    return (
        <div className="container mx-auto p-4">
            <h1 className="text-3xl font-bold mb-6 text-center">Manage Subjects (Courses)</h1> {/* Title adjusted */}
