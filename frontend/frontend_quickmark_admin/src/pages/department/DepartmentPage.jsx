...
export default function DepartmentPage({ departments, onAdd, onDelete, onSelectDepartment }) {
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {filteredDepartments.map((dept) => (
              <div key={dept.id} className="group relative">
                ...
                <button onClick={() => onDelete(dept.id)} className="absolute -top-2 -right-2 bg-red-600 p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-700">
