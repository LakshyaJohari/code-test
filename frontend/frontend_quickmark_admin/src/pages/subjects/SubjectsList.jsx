...
// --- START: AddEditSubjectModal Component ---
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold">{subject ? "Edit Subject" : "Add New Subject"}</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-800"><X size={24} /></button>
        </div>
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
            <div><label className="block text-sm font-medium text-gray-700">Subject Name</label><input type="text" name="name" value={formData.name} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-blue-500 focus:border-blue-500" /></div>
            <div><label className="block text-sm font-medium text-gray-700">Department</label><input type="text" name="department" value={formData.department} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-blue-500 focus:border-blue-500" /></div>
            <div><label className="block text-sm font-medium text-gray-700">Faculty</label><input type="text" name="faculty" value={formData.faculty} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-blue-500 focus:border-blue-500" /></div>
          </div>
          <div className="mt-6 flex justify-end space-x-3">
            <button type="button" onClick={onClose} className="bg-gray-200 text-gray-800 px-4 py-2 rounded-lg hover:bg-gray-300 font-semibold">Cancel</button>
            <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 font-semibold">Save</button>
          </div>
        </form>
      </div>
    </div>
...
// --- END: AddEditSubjectModal Component ---
...
export default function SubjectsList({
  ...
  const uniqueYears = useMemo(() => [...new Set(subjects.map(s => s.startYear).sort())], [subjects]);
  const uniqueDepartments = useMemo(() => [...new Set(subjects.map(s => s.department).sort())], [subjects]);
  const uniqueSubjects = useMemo(() => [...new Set(subjects.map(s => s.name).sort())], [subjects]);
  const uniqueFaculty = useMemo(() => [...new Set(subjects.map(s => s.faculty).sort())], [subjects]);
  ...
  const filteredSubjects = useMemo(() => {
    return subjects.filter((subject) => {
      ...
      const yearMatch = filterYear ? subject.startYear.toString() === filterYear : true;
      ...
      <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
          ...
          <div className="flex items-center space-x-2 w-full md:w-auto">
            ...
            <div className="relative flex-grow">
              ...
              <input type="text" placeholder="Search..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
            {/* Filter Button & Panel */}
              ...
              {isFilterOpen && (
                <div className="absolute right-0 mt-2 w-72 bg-white rounded-lg shadow-xl p-4 z-20 border">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="block text-sm font-medium text-gray-700 mb-1">Start Year</label><select value={filterYear} onChange={e => setFilterYear(e.target.value)} className="w-full p-2 border rounded-md text-sm"><option value="">All</option>{uniqueYears.map(y => <option key={y} value={y}>{y}</option>)}</select></div>
            ...
            <button onClick={handleAddClick} className="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 text-sm font-semibold">Add Subject</button>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead><tr className="border-b bg-gray-50"><th className="py-2 px-4">Subject Name</th><th className="py-2 px-4">Department</th><th className="py-2 px-4">Faculty</th><th className="py-2 px-4 text-right">Actions</th></tr></thead>
            <tbody>
              {filteredSubjects.map((subject) => (
                <tr key={subject.id} className="border-b text-sm hover:bg-gray-100 cursor-pointer" onClick={() => onViewDetails(subject)}>
                  <td className="py-3 px-4 font-medium text-gray-800">{subject.name}</td>
                  <td className="py-3 px-4 text-gray-600">{subject.department}</td>
                  <td className="py-3 px-4 text-gray-600">{subject.faculty}</td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={(e) => { e.stopPropagation(); handleEditClick(subject); }} className="text-blue-600 hover:underline font-semibold">Edit</button>
                  </td>
                </tr>
              ))}
            </tbody>
